...
configure:
	...
	@if [[ -z $${aws_profile} ]];then \
		while ! /usr/bin/grep -q "^\[$${aws_profile}\]$$" ~/.aws/credentials;do \
	...
	if [[ -z $${aws_region} ]];then \
		...
		while ! printf '%s\n' $${available_regions[@]} | /usr/bin/grep -q "^$${aws_region}$$";do \
	...
	if [[ -z $${bootstrap_stack} ]];then \
		...
		while [[ "$$(aws --profile $${aws_profile} --region $${aws_region} cloudformation describe-stacks --stack-name $${bootstrap_stack} --query 'Stacks[].StackName' --output text 2>/dev/null)" == "$${bootstrap_stack}" ]];do \
			LC_ALL=C rnd=$$(cat /dev/urandom | /usr/bin/tr -dc 'a-z0-9' | /usr/bin/fold -w 3 | /usr/bin/head -n 1); \
	...
	if [[ -z $${munki_repo} ]];then \
		while [[ "$$(aws --profile $${aws_profile} --region $${aws_region} codecommit get-repository --repository-name $${munki_repo} --query 'repositoryMetadata.repositoryName' --output text 2>/dev/null)" == "$$(echo $${munki_repo} | /usr/bin/tr "[:upper:]" "[:lower:]")" || -z $${munki_repo} || ! $${munki_repo} =~ ^[A-Za-z-]+$$ ]];do \
	...
	if [[ -z $${munki_s3_bucket} ]];then \
		while ! aws --profile $${aws_profile} s3 ls "s3://$${munki_s3_bucket}" 2>&1 | /usr/bin/grep -q 'NoSuchBucket';do \
	...
	if [[ -z $${codebuild_scripts_bucket} ]];then \
		...
		while ! aws --profile $${aws_profile} s3 ls "s3://$${codebuild_scripts_bucket}" 2>&1 | /usr/bin/grep -q 'NoSuchBucket';do \
			LC_ALL=C rnd=$$(cat /dev/urandom | /usr/bin/tr -dc 'a-z0-9' | /usr/bin/fold -w 3 | /usr/bin/head -n 1); \
	...
	if [[ -z $${munki_logging_bucket} ]];then \
		...
		while ! aws --profile $${aws_profile} s3 ls "s3://$${munki_logging_bucket}" 2>&1 | /usr/bin/grep -q 'NoSuchBucket';do \
			LC_ALL=C rnd=$$(cat /dev/urandom | /usr/bin/tr -dc 'a-z0-9' | /usr/bin/fold -w 3 | /usr/bin/head -n 1); \
...
deploy: requirements
	...
	@/usr/bin/zip -r $${zipfile} ./munki ./sync_and_makecatalogs.sh ./buildspec.yml
	@echo ""
...
destroy: requirements
	...
	echo -en "\n\n"; \
	echo -e "  \033[31mW A R N I N G  \xe2\x86\x92\033[0m  This action will \033[4mdestroy all resources\033[0m\n"; \
	...
	echo ""; \
	echo -e "  (Munki-specific IAM users need to be manually removed in\n"; \
	echo -e "  the AWS Console before destroying.)\n"; \
	echo "";\
	...
	bucket.object_versions.delete()' 2>&1); \
		...
		if echo $${empty_bucket} | /usr/bin/grep -q 'NoSuchBucket';then \
			echo -e " \033[33m[NOT FOUND]\033[0m"; \
			/usr/bin/tput cuu 1 && echo -e "\033[33m?\033[0m"; \
		elif echo $${empty_bucket} | /usr/bin/grep -q 'Access Denied';then \
			echo -e " \033[31m[ACCESS DENIED]\033[0m"; \
			/usr/bin/tput cuu 1 && echo -e "\033[31mx\033[0m\n"; \
			echo -e "  Exiting.."; \
			exit; \
		elif echo $${empty_bucket} | /usr/bin/grep -q 'Could not connect';then \
			...
			/usr/bin/tput cuu 1 && echo -e "\033[32m\xE2\x9C\x94\033[0m"; \
		fi; \
	done; \
	...
	for stack in $${stacks[@]};do \
		...
		if echo $${stack_status} | /usr/bin/grep -q 'does not exist';then \
			echo -e " \033[33m[NOT FOUND]\033[0m"; \
			/usr/bin/tput cuu 1 && echo -e "\033[33m?\033[0m"; \
		elif echo $${stack_status} | /usr/bin/grep -q 'Could not connect';then \
			echo -e " \033[31m[CONNECTION ERROR]\033[0m"; \
			/usr/bin/tput cuu 1 && echo -e "\033[31mx\033[0m\n"; \
			exit; \
			echo -e "  Exiting.."; \
			...
			/usr/bin/tput cuu 1 && echo -e "\033[34m\xE2\x9C\x94\033[0m"; \
		fi; \
	done; \
	echo ""
